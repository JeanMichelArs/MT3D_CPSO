...
#valeur modele
### rho,nx,ny,nz,hx,hy,hz = mf.read_model_mackie(file)
#Indice
ind=5
...
# Coefficient MISFIT MT
mum=input("MT misfit damping coefficient")

# Coefficient MT REGULARISATION
lambdam=input("MT regularisation damping coefficient")
...
#-----------------------------------------#
#                                         #
#  A COMMON VECTOR MODEL & HALSF-PACE1D   #
#                                         #
#-----------------------------------------#

Xo=np.hstack((rho,ini1D[:,0],ini1D[:,1]))
"""
...
def F(X):
    ...
    rest=10**X[model-1]
    print rest
    cost=mum*XHI2(rest)+lambdam*REG(rest)
    return cost
...
def XHI2(X):
    ...
    #----------------
        ...
        # IMPEDANCE TENSOR
        """
        zxxd=RZxx[ii]+1j*IZxx[ii]
        Ezxxd=ErZxx[ii]
        zyyd=RZyy[ii]+1j*IZyy[ii]
        Ezyyd=ErZyy[ii]
        zxyd=RZxy[ii]+1j*IZxy[ii]
        Ezxyd=ErZxy[ii]
        zyxd=RZyx[ii]+1j*IZyx[ii]
        Ezyxd=ErZyx[ii]
        """
        ...
        resp_f=open('RhoPhi_'+idd+'.resp',"w")
...
#Xi=np.log10(rho)
lower=np.ones(n_dim)*1  #Xi-np.ones(n_dim)*2
upper=np.ones(n_dim)*2  #Xi+np.ones(n_dim)*2

popsize=input("Size of model SWARM:")
max_iter=input("Number of iteration:")

# Initialize SOLVER
ea = Evolutionary(F,lower = lower, upper = upper, popsize = popsize, max_iter = max_iter,eps1 = 1e-1, eps2 = 1e-8,snap = True)
...
#COMPUTE F with the solution array
xopt=ea.xopt
F(xopt)
#print np.around(xopt,1)
print (ea)
###print 'Solution:',ea.xopt
###print 'Fitness:',ea.gfit
###print 'Number of iteration:',ea.n_iter
###print 'Number of call to F:',ea.n_eval
###print 'Flag:',ea.flag
###
...
#-----------------------------------#
#                                   #
# WRITING FINAL RESISTIVITY MODEL   #
#                                   #
#-----------------------------------#
...
filefmt=open('RESITIVITY_3dc.rslt',"w")
