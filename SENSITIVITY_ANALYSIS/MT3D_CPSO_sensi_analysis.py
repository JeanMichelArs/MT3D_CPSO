...
def XHI2(X):
    ...
    for n in range(len(lab)):
        ang=rot[n]
        ang=ang*np.pi/180
        rotb=np.array([[np.cos(ang),np.sin(ang)],[-np.sin(ang),np.cos(ang)]])
        ss=site_id[n]
    ...
    #----------------------------------------------------------------------------
        for pol in range(2):
            ...
            EF=np.array([exr,eyr])
            HF=np.array([hxr,hyr])
            EFr=np.matmul(rotb,EF)
            HFr=np.matmul(rotb,HF)
            exr=EFr[0,:]
            eyr=EFr[1,:]
            hxr=HFr[0,:]
            hyr=HFr[1,:]
    ...
    #----------------
        ...
        RMS1r[n]=sum((np.log10(rhoxx_d)-np.log10(rhoxx_ci))**2)+ \
             sum((np.log10(rhoxy_d)-np.log10(rhoxy_ci))**2)+ \
             sum((np.log10(rhoyx_d)-np.log10(rhoyx_ci))**2)+ \
             sum((np.log10(rhoyy_d)-np.log10(rhoyy_ci))**2)
        RMS1p[n]=sum((phixx_d-phixx_ci)**2)+ \
             sum((phixy_d-phixy_ci)**2)+ \
             sum((phiyx_d-phiyx_ci)**2)+ \
             sum((phiyy_d-phiyy_ci)**2)
    ...
    print 'RMS Resistivity apparent => RMS1r=',RMSrr,'Ohm.m'
