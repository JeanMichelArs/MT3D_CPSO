...
iterpercent_range = np.array([10, 30, 50, 65, 75, 85, 100], dtype=int)

"""
nruns_range = np.array([100]) 
iterpercent_range = np.array([100]) 
"""

for nruns in nruns_range:
    for iterpercent in iterpercent_range:
        print ''
        print 'Computing stat for '+ str(iterpercent) + \
              ' percent of total iteration of ' + str(nruns) + " runs merged"
        print '---------------------------------------------------------------'
        # ---------------------------------------------------------------------
        ...
        """
        run = 'Calibre_CPSO_8nz'
        NCPATH = '/postproc/COLLIN/MTD3/'
        ncfile = NCPATH + run + '/merged.nc'
        data_file = '/home/j/jcollin/MT3D_CPSO/Config/1D/model_001/001.ro'

        folder_save = NCPATH + '/convergence_rms/' 
        save_plot = True
        outfile = folder_save + "/mselect_mod_nruns" + str(nruns) \
                  + '_iterpercent' + str(iterpercent) + ".nc"
        save_netcdf = True              
        ...
        upper = 2.
        kappa = 1               
        pdf_m, n_bin, x_bin = pp.marginal_law(m_grid, f_grid, logrhosynth,
