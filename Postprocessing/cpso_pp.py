...
def NaN_filter(models, energy,iterpercent=100, timing=True, **kwargs):
    ...
    """ Add iterpercent variable to select a % of total iter """
    if timing:
        t0 = time.clock()
    if len(models.shape) == 3:
        popsize, nparam, nitertot = models.shape
        tmp = energy[energy < 1e20 ]
        niter = len(tmp) // popsize
...
def value_filter(models, energy, threshold, timing=True, **kwargs):
    ...
    if len(models.shape) == 3:
        ...
        "! in some cases minimum may be found multiple times, we need to pick one"
        ...
        if log:
            for iparam in range(nparam):
                std_weight[iparam] = np.sqrt((nmodel / (nmodel-1)) \
                                   ...
                                   * (m_grid[:, iparam] - m_weight[iparam])**2) * S)
        else: 
            for iparam in range(nparam):
                std_weight[iparam] = np.sqrt((nmodel / (nmodel-1)) \
                                   * np.sum(np.exp(f_grid) \
...
def marginal_law(m_grid, f_grid, m_best, ndata, n_inter=30, lower=-1, upper=1, 
    ...
    if rms:
        ...
        S = 1 / np.sum(np.exp((f_best - f_grid) * lmbda))
    for iparam in range(nparam):
       for i_inter in range(n_inter):
            p_inter = lower + i_inter * (upper-lower) / (n_inter - 1) + m_best[iparam]
