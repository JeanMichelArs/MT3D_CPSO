...
ld = linecache.getline(mod_rho, 1)
lx = linecache.getline(mod_rho, 2)
ly = linecache.getline(mod_rho, 3)
lz = linecache.getline(mod_rho, 4)
dim=ld.split()
nx=int(dim[0])
ny=int(dim[1])
nz=int(dim[2])
#valeur modele
ind=4+(nz)+(nz-1)*ny+(ny)
l=linecache.getline(mod_rho, ind+1)
...
for k in np.arange(nz):
    ...
    for i in np.arange(ny):
        ind=4+(k+1)+(k)*ny+(i+1)
        l=linecache.getline(mod_rho, ind)
...
ld = linecache.getline(mod_param, 1)
lx = linecache.getline(mod_param, 2)
ly = linecache.getline(mod_param, 3)
lz = linecache.getline(mod_param, 4)
dim=ld.split()
nx=int(dim[0])
ny=int(dim[1])
nz=int(dim[2])
#valeur modele
ind=4+(nz)+(nz-1)*ny+(ny)
l=linecache.getline(mod_param, ind+1)
...
for k in np.arange(nz):
    ...
    for i in np.arange(ny):
        ind=4+(k+1)+(k)*ny+(i+1)
        l=linecache.getline(mod_param, ind)
        L=np.vstack(float(l.split()[i]) for i in np.arange(nx) )
        L.shape=(nx,)
...
nc = Dataset(nc_file, "r", format="NETCDF4")
#models history
...
plt.savefig(out_fig1, format='png',dpi=300)
...
plt.savefig(out_fig2, format='png',dpi=300)
